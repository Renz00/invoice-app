...
//Assigning props from show.vue to a props variable with type String
...
const getInvoice = async () => {
    //use backticks ` for using props varible in url
    let res = await axios.get(`/api/edit_invoice/${props.id}`)
    //indicated the index so that the value can be passed to the form object
    form.value = res.data.invoice[0]
}
...
const addCart = (item) => {
    ...
    //sets default value 1 for quantity to avoice computation errors
    itemcart.quantity = 1
...
const subTotal = () => {
    ...
    //checks if invoice_items object is not empty
        ...
        <div class="card__content">
            ...
            <div class="table">
                ...
                <div class="table--items2" v-for="(itemcart, index) in form.invoice_items" :key="itemcart.id">
                    <!-- This object has duplicate data? idk why this if statement is needed  -->
                ...
                <div style="padding: 10px 30px !important;">
                    <button class="btn btn-sm btn__open--modal" @click="modalToggle(true)">Add New Line</button>
                </div>
            </div>
